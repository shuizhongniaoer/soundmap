...
const rows = [
  {
    ...
    sprouts: { items: [{ title: '发布不是终点', source: '下周发布新版本', seedSummary: '一次出发', reference: '忒修斯之船', echo: '船在航行中被持续更换木板', expansion: '发布之后才进入学习循环', aha: '版本要靠反馈生长' }] },
  },
...
test('searches title, transcript, speaker and summary text', () => {
  ...
  assert.deepEqual(searchRecordings(rows, '学习循环').map(x => x.rec.id), ['a']);
  assert.deepEqual(searchRecordings(rows, '忒修斯之船').map(x => x.rec.id), ['a']);
});
