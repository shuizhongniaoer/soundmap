// Prompt 模板（对应产品设计 3.3 节：多维总结 + 场景模板 + 思维导图）
// 核心策略：用户可指定场景模板（会议/销售/课堂/访谈/灵感），按模板风格总结；
// 默认 auto 模式让模型自动判断类型再选风格。
...
2. key_points 对生活对话来说是"有趣的瞬间、重要的信息、值得回忆的细节"，不是"决议事项"。`;
  ...
  const systemContent = tpl
    ? `你是录音笔记助手。${tpl.instruction}
...
- quotes: 值得记住的原话数组
- qa_pairs: 问答对数组，每项 {q, a}，从录音中提取实际出现的问答（有人提问有人回答），没有则返回空数组 []

通用规则：
1. todos 只在确实有人约定或计划要做某件事时才填，没有就返回空数组 []，绝不为了填满字段而编造。
2. qa_pairs 只提取录音中真实发生的问答（一方提问、另一方回答），不要把总结改写成问答形式；没有真实问答就返回 []。
3. 转写稿可能存在语音识别错误，结合上下文推断真实意思，明显误听的词不要照抄进总结。`
    : `你是录音笔记助手。先判断录音内容的类型，再按类型用合适的风格总结。
...
4. qa_pairs 只提取录音中真实发生的问答（一方提问、另一方回答），不要把总结改写成问答形式；没有真实问答就返回 []。
5. 转写稿可能存在语音识别错误，结合上下文推断真实意思，明显误听的词不要照抄进总结。`;
